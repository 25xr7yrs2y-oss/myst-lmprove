...
- Proxy mode skips peer route exclusion at the P2P semantic call site and selects
  a no-op routing backend, so `--usermode --proxymode` never requires the
  supervisor pipe or observes/modifies host routes.
- Initial gateway discovery for actual system-tunnel modes is cancelable and
  bounded to five attempts with exponential backoff. Exhaustion returns a
  specific error instead of spinning forever.
...
The workflow also injects the exact GitHub source commit, ref, run identifier,
and backend release version into `myst.exe`, verifies `myst.exe --version`, and
publishes a SHA-256 provenance record beside Privacy Browser backend tag assets.
